...
2. **界面全景梳理**
   - 项目包含哪些主要界面（Activity/Fragment/Composable）？请提供所有关键界面的类名及其完整文件路径。
...
5. **自定义组件全景分析**
   - 项目中所有自定义组件（View/ViewGroup/Composable）的完整清单是什么？请提供每个组件的类名、完整文件路径和行号范围。
   - 每个自定义组件的核心实现原理是什么？请指出关键绘制方法、测量方法和事件处理方法的位置。
   - 自定义组件是如何被其他界面复用的？请列出调用位置和使用方式。
...
3. **导航实现全景分析**
   - 项目中导航系统的完整实现方式是什么？请提供所有导航相关类、接口和配置文件的位置。
   - 页面跳转的具体代码实现在哪里？请列出所有跳转方法的位置和调用关系。
   - 有无自定义导航框架？如有，请详细分析其核心类和实现原理，提供关键代码位置。
   - 应用的启动流程和深层链接处理逻辑是怎样的？请提供入口代码和流程实现位置。
...
4. **线程与并发**
   - 项目如何处理后台任务？请指出线程池或协程调度器的配置位置。
   - 如何避免主线程阻塞？请列出关键异步操作的实现位置。
   - 项目中是否使用 Kotlin 协程？请提供关键协程作用域和上下文定义的位置。
...
1. **整体架构**
   ...
   - 模块间依赖如何管理？请指出依赖关系配置的位置。

2. **设计模式**
   - 项目中使用了哪些设计模式？请列出每种设计模式实现的具体类和文件位置。
   - 如何实现代码的可测试性和可维护性？请指出关键抽象和接口的位置。
   - 项目是否使用了 Clean Architecture 或 SOLID 原则？请提供相关实现示例。
...
1. **第三方库集成**
   ...
   - 有无自研的核心基础库？请提供其实现位置和使用方式。

2. **资源管理**
   - 项目如何组织和管理资源文件？请列出关键资源文件的路径。
   - 如何支持多语言和主题切换？请提供相关配置和实现代码的位置。
   - 如何处理不同屏幕密度的图片资源？请指出相关资源目录结构。

3. **安全性**
   - 项目如何处理敏感数据？请指出加密或安全存储实现的代码位置。
   - 是否实现了代码混淆和加固？请提供相关配置文件的位置和关键配置。
   - 如何防止常见的安全漏洞（如注入攻击、MITM）？请指出安全策略实现位置。

4. **应用特有功能**
   - 项目有哪些核心功能和特色？请列出相关实现类和方法的位置。
   - 如何实现特定领域的业务逻辑？请提供核心算法或逻辑的代码位置。
   - 项目如何处理特殊硬件或系统功能（如相机、传感器、通知等）？请指出相关代码位置。
