...
| **构建系统** | Gradle | 8.0+ | 使用 Gradle 作为构建工具，管理依赖和构建流程 |
...
* `androidTest`：包含应用程序的 Android 测试代码。
* `main/java`：包含主要的 Kotlin 源代码文件。
* `ui`：包含界面层代码，包括 Composable 组件、屏幕和主题。
* `domain`：包含业务层代码，包括视图模型、用例和业务模型。
* `data`：包含数据层代码，包括仓库、数据源和数据模型。
* `res`：包含资源文件，如图像、字符串和主题。
* `test`：包含单元测试代码。
...
* `AppTopBar`：应用顶部工具栏，通常包含标题和操作按钮。
* `NavHost`：导航宿主，负责管理不同屏幕之间的导航。
* `HomeScreen`：主页屏幕，通常是应用的首页。
* `DetailScreen`：详情屏幕，用于显示项目的详细信息。
* `ContentList`：内容列表组件，用于展示多个内容项。
* `ContentItem`：单个内容项组件，列表中的基本单位。
